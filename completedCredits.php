		...
		<div class="panel panel-primary mainbody">
		  ...
		  <?php include("billing_nav.php"); ?>
			<!-- <ul class="nav nav-tabs">
			  ...
			  <li role="presentation" class="active"><a href="completedCredits.php">Completed Credits</a></li>
			...
			<div class='well'>
				<div class='row'>
			<form action='completedCredits.php' method='get'>
						...
						<?php 
							...
							if( isset($_GET['submit']) ){
								...
								if( !empty($date1) && !empty($date2) ){
									...
									$qry = "WHERE (`date` BETWEEN '$date1' AND '$date2') AND status='1' AND transaction='In' AND fullPayment='Partial' AND clearanceStatus='1' AND login='$login'";
								}else{
									$qry = "WHERE status='1' AND transaction='In' AND clearanceStatus='1' AND fullPayment='Partial' AND login='$login'";
								}
							}else{
								$qry = "WHERE date='".date('Y-m-d')."' AND status='1' AND transaction='In' AND clearanceStatus='1' AND fullPayment='Partial' AND login='$login'";
							}	
							...
							$get = mysqli_query($db, $sql);
								...
								while( $res = mysqli_fetch_array($get) ){
									...
									$paidSoFar = $partialPayment + checkPendingBalance($id, $db);
									...
									echo "<td>$qty</td>";
									echo "<td>$total</td>";
									echo "<td>".$paidSoFar."</td>";									
									echo "<td class='text-danger'><strong>".$pendingAmount."</strong></td>";									
