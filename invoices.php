						...
						<?php 
							...
							if( isset($_GET['submit']) ){
								...
								if( !empty($date1) && !empty($date2) ){
									...
									$qry = "WHERE (`date` BETWEEN '$date1' AND '$date2') AND status='1' AND login='$login'";
								}else{
									$qry = "WHERE status='1' AND login='$login'";
								}
							}else{
								$qry = "WHERE date='".date('Y-m-d')."' AND status='1' AND login='$login'";
							}	
							...
							$get = mysqli_query($db, $sql);
								...
								while( $res = mysqli_fetch_array($get) ){
									...
									echo " <a class='btn btn-xs btn-primary' href='ewaybill.php?invoice=$id'><span class='glyphicon glyphicon glyphicon-road'></span></a> ";
