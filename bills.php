						...
						<?php 
							...
							if( isset($_GET['submit']) ){
								...
								if( !empty($date1) && !empty($date2) ){
									...
									$qry = "WHERE (`date` BETWEEN '$date1' AND '$date2') AND status='1' AND transaction='In' AND login='$login'";
								}else{
									$qry = "WHERE status='1' AND transaction='In' AND login='$login'";
								}
							}else{
								$qry = "WHERE date='".date('Y-m-d')."' AND status='1' AND transaction='In' AND login='$login'";
							}	
