	...
	<div class='row'>
		<div class="panel panel-primary mainbody">
		  ...
		  <?php include("billing_nav.php"); ?>
			...
			<div class='well'>
				<div class='row'>
			<form action='pending_invoices.php' method='get'>
						...
						<?php 
							...
							if( isset($_GET['submit']) ){
								...
								if( !empty($date1) && !empty($date2) ){
									...
									$qry = "WHERE (`date` BETWEEN '$date1' AND '$date2') AND status='0' AND transaction='In' AND login='$login'";
								}else{
									$qry = "WHERE status='0' AND transaction='In' AND login='$login'";
								}
							}else{
								$qry = "WHERE date='".date('Y-m-d')."' AND status='0' AND transaction='In' AND login='$login'";
							}	
							...
							$get = mysqli_query($db, $sql);
								...
								while( $res = mysqli_fetch_array($get) ){
									...
									echo "<a class='btn btn-xs btn-info' href='estimate.php?estimateId=$id'><span class='glyphicon glyphicon-zoom-in'></span></a> ";
